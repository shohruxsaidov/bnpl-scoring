...
**Merchant App and Admin Portal target 450px minimum width — fully responsive down to phone screens.** The primary hardware context remains counter terminals (laptop) and tablets handed to Clients during MyID verification, but both portals must be fully usable on any screen ≥ 450px. The Deal Wizard stepper collapses to an icons-only row with a "N / 7 — StepName" label below 450px. Data tables use horizontal scroll (overflow-x: auto) rather than column hiding or card-list transforms. The `body { min-width: 768px }` hard lock has been removed from the merchant portal. A dedicated native mobile app remains rejected — a responsive web app covers the real device range without a separate codebase and app-store overhead.
