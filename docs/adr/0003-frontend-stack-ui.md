...
**New Deal Wizard uses SSE for external API progress.** The wizard streams step-level progress (MyID, KATM, Plum) via Server-Sent Events. Each step shows pending/in-progress/succeeded/failed with a per-step retry button on failure. Synchronous HTTP rejected — sequential external calls can take 20+ seconds, exceeding acceptable timeouts. Async job queue with polling rejected as more infrastructure than needed. SSE maps directly onto the step-level progress UI with no polling overhead and no WebSocket handshake complexity.
