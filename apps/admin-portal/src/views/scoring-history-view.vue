...
const merchantOptions = computed(() => [
  { label: t('scoringHistory.allMerchants'), value: null },
...
const filtered = computed<ScoringListItem[]>(() => {
  ...
  if (search.value.trim()) {
    ...
    list = list.filter(
      (r) => r.clientName.toLowerCase().includes(q) || r.clientPhone.includes(q),
    )
  }
  return list
})
...
<template>
  <div class="page">
    ...
    <div v-else-if="store.error" class="surface-card error-state">
      ...
      <p>{{ store.error }}</p>
      <button class="btn-ghost" @click="store.fetchList(merchantFilter ?? undefined)">{{ $t('common.retry') }}</button>
    </div>
