...
import { scoringModels, scoringModelRevisions } from '../modules/admin/scoringModel/schema';
...
async function main() {
  ...
  let [global] = await db
    ...
    .limit(1);
  if (!global) {
    const [created] = await db.insert(scoringModels).values({ name, isGlobal: true }).returning();
    global = created!;
  }
