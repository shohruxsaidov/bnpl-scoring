...
import { listBlacklist } from './queries/list-blacklist';
import { getAdminById } from './queries/get-admin';
import { addBlacklistEntry } from './commands/add-to-blacklist';
import { removeBlacklistEntry } from './commands/remove-from-blacklist';
...
export default async function adminBlacklistRoutes(app: FastifyInstance) {
  ...
  fastify.delete('/:id', { schema: { params: IdParams }, preHandler }, async (request, reply) => {
    const entry = await removeBlacklistEntry(db, +request.params.id);
