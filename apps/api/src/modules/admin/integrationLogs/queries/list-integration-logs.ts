...
import { integrationLogs } from "../../../integrations/db/schema"
...
function errorCondition(): SQL {
  return or(
    isNotNull(integrationLogs.errorMessage),
    gte(integrationLogs.status, 400),
  )!
}
...
export async function listIntegrationNames(db: Db): Promise<string[]> {
  const rows = await db
