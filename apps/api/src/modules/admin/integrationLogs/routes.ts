...
} from "./queries/list-integration-logs"
import { getIntegrationLog } from "./queries/get-integration-log"
...
export default async function adminIntegrationLogRoutes(app: FastifyInstance) {
  ...
  fastify.get("/", { schema: { querystring: ListQuery }, preHandler }, async (request) => {
    ...
    const { logs, total } = await listIntegrationLogs(db, {
  ...
  fastify.get("/integrations", { preHandler }, async () => {
    const integrations = await listIntegrationNames(db)
    return { integrations }
  })
  ...
  fastify.get("/:id", { schema: { params: IdParams }, preHandler }, async (request, reply) => {
    ...
    const log = await getIntegrationLog(db, request.params.id)
