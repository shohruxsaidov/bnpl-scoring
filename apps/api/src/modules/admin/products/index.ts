...
import { getProduct } from "./queries/get-product/get-product.handler"
  ...
  fastify.patch(
    ...
    async (request, reply) => {
      ...
      if (categoryId !== undefined) {
        const enabled = await isCategoryEnabledForMerchant(categoryId, product.merchantId)
        if (!enabled) return reply.code(400).sendError("category_not_enabled")
      }
      const updated = await updateProduct({ id: product.id, ...request.body, categoryId })
