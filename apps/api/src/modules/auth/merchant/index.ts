...
function buildAccessToken(
  app: FastifyInstance,
  employee: { id: number; merchantId: number; branchId: number },
  role: string,
  roleId: number,
): string {
  return app.jwt.sign(
...
async function resolveRoleByKey(roleKey: string) {
...
export default async function merchantAuthRoutes(app: FastifyInstance) {
  ...
  fastify.post('/login', { schema: { tags: TAGS, body: LoginBody } }, async (request, reply) => {
    ...
    if (roles.length === 1) {
      const resolved = await resolveRoleByKey(roles[0]!);
      if (!resolved) return reply.code(500).sendError('role_not_configured');
      const { sessionToken } = await createMerchantSession(employee.id, roles[0]!);
  ...
  fastify.post('/select-role', { schema: { tags: TAGS, body: SelectRoleBody } }, async (request, reply) => {
    ...
    const employee = await findMerchantUserById(+payload.employeeId);
    if (!employee || !employee.active) {
      return reply.code(401).sendError('invalid_credentials');
    }

    const resolved = await resolveRoleByKey(role);
    if (!resolved) return reply.code(500).sendError('role_not_configured');

    const { sessionToken } = await createMerchantSession(employee.id, role);
  ...
  fastify.post('/refresh', { schema: { tags: TAGS } }, async (request, reply) => {
    ...
    const result = await verifyMerchantSession(sessionToken);
    if (!result) {
      clearAuthCookies(reply);
      return reply.code(401).sendError('unauthorized');
    }

    const resolved = await resolveRoleByKey(result.session.selectedRole);
    if (!resolved) {
      clearAuthCookies(reply);
      return reply.code(401).sendError('unauthorized');
    }
  ...
  fastify.get('/me', { schema: { tags: TAGS }, preHandler: app.verifyMerchantJwt }, async (request, reply) => {
    ...
    const resolved = await resolveRoleByKey(payload.role);
    if (!resolved) return reply.code(500).sendError('role_not_configured');
    return {
      user: serializeEmployee(employee, payload.role, resolved.role.id, resolved.permissions),
    };
  });
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      await changeMerchantPassword(employee.id, request.body.newPassword);
      return { ok: true };
    },
  );
