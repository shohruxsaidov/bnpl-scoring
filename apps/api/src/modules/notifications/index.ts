...
export default async function notificationsModule(app: FastifyInstance) {
  ...
  fastify.get('/', { preHandler: verifyAny }, async (request) => {
    ...
    const rows = await listNotifications(actorType, actorId)
  ...
  fastify.patch(
    ...
    async (request) => {
      const { actorType, actorId } = actorFrom(request.user as AnyPayload)
      await markOneRead(request.params.id, actorType, actorId)
      return { ok: true }
    },
  )
  ...
  fastify.post('/read-all', { preHandler: verifyAny }, async (request) => {
    const { actorType, actorId } = actorFrom(request.user as AnyPayload)
    await markAllRead(actorType, actorId)
    return { ok: true }
  })
  ...
  fastify.delete('/', { preHandler: verifyAny }, async (request) => {
    const { actorType, actorId } = actorFrom(request.user as AnyPayload)
    await deleteRead(actorType, actorId)
    return { ok: true }
  })
