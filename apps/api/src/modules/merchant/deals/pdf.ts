...
const STRINGS = {
  ru: {
    ...
    note5:
      '5. Стороны не имеют взаимных претензий к качеству товара и (или) предоставленной услуге.',
  ...
  uz: {
    appendixRef: (date: string) =>
      `Mahsulotni yetkazib berish bo'yicha\nOmmaviy ofertaga ${date} dagi Ilova`,
    ...
    note6:
      '6. Ushbu Dalolatnoma har bir tomon uchun bittadan, bir xil yuridik kuchga ega 2 (ikki) nusxada tuzildi.',
  ...
  for (let ri = 0; ri < rows.length; ri++) {
    ...
    for (let ci = 0; ci < row.length; ci++) {
      ...
      const align: 'left' | 'center' | 'right' =
        colAlignments?.[ci] ?? (ci === 0 ? 'center' : ci === row.length - 1 ? 'right' : 'left');
...
function drawTotalRow(
  ...
  for (let i = 0; i < cells.length; i++) {
    ...
    const cell = cells[i] ?? '';
    const align: 'left' | 'center' | 'right' =
      i === 0 ? 'center' : i === cells.length - 1 ? 'right' : 'left';
...
export async function generateContract(data: ContractData, lang: 'ru' | 'uz'): Promise<Buffer> {
  ...
  doc
    .moveTo(MARGIN + colW + 8, y)
    .lineTo(MARGIN + colW + 8, y + partiesH)
    .stroke('#cccccc');
  doc.restore();
  ...
  const drawPartyLines = (lines: Array<[string, string]>, xOff: number) => {
    ...
    for (const [label, val] of lines) {
      doc
        .font('Bold')
        .fillColor('#1a1a1a')
        .text(label + ' ', xOff, ly, { continued: true, lineBreak: false, width: colW - 6 });
  ...
  y = drawMergedTotalRow(
    ...
    [
      {
        text: t.totalLabel,
        width: (pColW[0] ?? 0) + (pColW[1] ?? 0) + (pColW[2] ?? 0),
        align: 'center',
      },
  ...
  doc.text(`${t.deliveredBy} ${data.branchName} (${data.merchantInn})`, MARGIN, sigTop + sigLineH, {
    width: sigW,
  });
  ...
  doc.text(t.stamp1, MARGIN + 4, stampY + 18, {
    width: stampW - 8,
    align: 'center',
    characterSpacing: 2,
  });
  ...
  doc.text(t.stamp2, rStampX + 4, stampY + 18, {
    width: stampW - 8,
    align: 'center',
    characterSpacing: 2,
  });
