...
// ---------------------------------------------------------------------------
export default async function publicUserPhotoRoutes(app: FastifyInstance) {
  ...
  fastify.get(
    ...
    async (request, reply) => {
      const objectKey = `user-photos/${request.params.objectKey}`;

      const [file] = await db.select().from(files).where(eq(files.objectKey, objectKey)).limit(1);
      if (!file) return reply.code(404).sendError('photo_not_found');
