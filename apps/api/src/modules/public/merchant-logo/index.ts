...
export default async function publicMerchantLogoRoutes(app: FastifyInstance) {
  ...
  fastify.get(
    '/public/merchants/:id/logo',
    {
      schema: {
        tags: TAGS,
        summary: 'Merchant logo image',
        params: Params,
    ...
    async (request, reply) => {
      ...
      if (!Number.isInteger(merchantId)) {
        return reply.code(404).sendError('merchant_logo_not_found');
      }
      ...
      if (!row?.logoFileId) return reply.code(404).sendError('merchant_logo_not_found');

      const [file] = await db.select().from(files).where(eq(files.id, row.logoFileId)).limit(1);
