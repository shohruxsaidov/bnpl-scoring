...
//
// credit_limit is whole som as a string, e.g. '1000000' ('0' on any rejection).
// Within [scored_at, expires_at) the value is reused and a re-score is skipped
// (no fresh KATM charge); after expires_at the app offers a manual refresh.
// ---------------------------------------------------------------------------
export const userCreditLimits = pgTable('user_credit_limits', {
  userId: integer('user_id')
    .primaryKey()
    .references(() => users.id),
  // Per-month disposable figure in whole som (string), as produced by
  // runModelAndLimit. '0' on any rejection (the reason lives on the scorings run).
  creditLimit: varchar('credit_limit', { length: 20 }).notNull(),
