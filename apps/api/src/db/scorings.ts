...
export const scorings = pgTable(
  'scorings',
  {
    ...
    dealSessionId: uuid('deal_session_id')
      .notNull()
      .references(() => dealSessions.id),
    // Denormalized from the deal session for admin filtering.
  ...
  (t) => [uniqueIndex('scorings_deal_session_idx').on(t.dealSessionId)],
);
