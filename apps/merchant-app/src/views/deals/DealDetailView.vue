...
const timeline = computed((): TimelineEvent[] => {
  ...
  return relevant.map((e, i) => ({
    ...
    icon: e.icon,
    time: e.time,
    variant: i < lastIndex ? 'done' : currentStatus === 'declined' ? 'current' : i === lastIndex ? 'current' : 'done',
  }))
})

// ── Учёт по сделкам (accounting ledger) ───────────────────────────────────
...
const nextPayment = computed(() => schedule.value.find((r) => r.status === 'pending') ?? null)
</script>
...
<template>
  <div v-if="deal" class="page">
    ...
    <!-- ── Сделки ──────────────────────────────────────────────────────── -->
    <div v-if="activeTab === 'sdelki'" class="surface-card deal-card">
    ...
    <!-- ── Контракты ───────────────────────────────────────────────────── -->
    <div v-else-if="activeTab === 'kontrakty'" class="surface-card deal-card">
      <div class="section">
    ...
    <!-- ── Учёт по сделкам ─────────────────────────────────────────────── -->
    <div v-else-if="activeTab === 'uchet'" class="surface-card deal-card">
      <div class="section">
    ...
    <!-- ── Просрочки ───────────────────────────────────────────────────── -->
    <div v-else-if="activeTab === 'prosrochki'" class="surface-card deal-card">
      <div class="section">
    ...
    <!-- ── Автоплатёж ──────────────────────────────────────────────────── -->
    <div v-else-if="activeTab === 'avtoplatezh'" class="surface-card deal-card">
      <div class="section">
